...
export default function ShareItinerary() {
  ...
  useEffect(() => {
    const fetchSharedItinerary = async () => {
      ...
      try {
        setIsLoading(true);
        setError(null);
        const data = await itineraryService.getByShareToken(token);
        setItinerary(data);
      } catch (err: any) {
        setError(err.response?.data?.message || 'Shared itinerary not found or has been set to private.');
      } finally {
        setIsLoading(false);
      }
    };
    ...
    return (
      <div className="min-h-screen bg-surface-900 text-slate-100 flex flex-col items-center justify-center p-6">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-slate-400 font-medium">Fetching itinerary plan...</p>
        </div>
      </div>
    );
  }
  ...
  if (error || !itinerary) {
    return (
      <div className="min-h-screen bg-surface-900 text-slate-100 flex items-center justify-center p-6">
        <div className="max-w-md w-full glass p-8 rounded-3xl border-rose-500/20 text-center space-y-5">
          <h4 className="font-display font-bold text-xl text-white">Access Denied / Expired</h4>
          <p className="text-rose-400 text-sm leading-relaxed">{error || 'This itinerary is no longer publicly available.'}</p>
          <div className="pt-2">
  ...
  return (
    <div className="min-h-screen bg-surface-900 text-slate-100 relative pb-12">
      ...
      {/* Main Container */}
      <div className="max-w-5xl w-full mx-auto px-6 pt-10 space-y-8 relative z-10">
        
        {/* Branding Header */}
        <div className="flex items-center justify-between border-b border-slate-800 pb-5">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl gradient-brand flex items-center justify-center font-display font-bold text-white text-lg">
              T
            </div>
          ...
          <Link to="/register" className="btn-primary text-xs py-2 px-4 shadow-sm shadow-brand-500/10">
            Create Free Account
          </Link>
        </div>
                ...
                <div className="relative border-l border-slate-800/80 ml-3 space-y-8 pl-6">
                  {selectedDay.activities.map((activity, actIdx) => (
                    <div key={actIdx} className="relative group">
                      {/* dot */}
                      <div className="absolute left-[-31px] top-1.5 w-4 h-4 rounded-full bg-slate-900 border-2 border-brand-500 flex items-center justify-center">
            ...
            {itinerary.tips && itinerary.tips.length > 0 && (
              <div className="glass rounded-3xl border-white/5 p-6 space-y-4">
                <div className="flex items-center gap-2 border-b border-white/5 pb-3">
                  ...
                  <h3 className="font-display font-bold text-base text-white">AI Travel Tips</h3>
                </div>
